...
The command supports `--minutes`, `--codex-home`, `--data-path`, `--format`, `--out`, `--interval`, `--force-refresh`, `--save-history`, `--history`, `--help`, and `-h`. [cli-reference.md](./cli-reference.md) contains defaults, constraints, and examples.
