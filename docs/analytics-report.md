...
The event table keeps the default visible columns focused on timestamp, session metadata, model with intelligence level, observed token volume, effective input tokens, cached input tokens, cache hit rate, output tokens, and reasoning output tokens. Model values display as `model/intelligence`, such as `gpt-4.5/low`. Session values are displayed as the first hash section after the rollout timestamp. Numeric column headers and values are right-aligned. The Turn Index column is sized for compact numeric values and its header wraps onto two lines. Raw totals, visible output, reasoning rate, input tokens, and the short source file reference are available in expandable detail rows. Full session file paths appear in the Session Paths card at the bottom of the report.
