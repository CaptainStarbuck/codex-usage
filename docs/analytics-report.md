...
The text report is optimized for terminal review. It includes the selected window, quota details when available, aggregate totals, warnings and notices, and fixed-width event detail tables grouped by source session file. Its Summary section uses the same input, output, and totals grouping as the HTML Summary cards. Summary, quota, and event table labels are display labels, and Total Tokens uses observed token volume. Report window timestamps, quota capture timestamps, and event table timestamps use `DATETIME_FORMAT`. Text event details use the expanded HTML Sessions detail columns and add a Totals row at the end of each session file table.
