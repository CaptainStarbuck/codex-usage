  ...
  <script>
    ...
    function readEventTableState() {
      try {
        ...
        return {
          expanded: Array.isArray(state.expanded) ? state.expanded.filter((key) => typeof key === 'string') : [],
          sortColumn: typeof state.sortColumn === 'string' ? state.sortColumn : undefined,
          sortDirection: Number(state.sortDirection) === -1 ? -1 : 1
        };
      } catch {
        return { expanded: [], sortColumn: undefined, sortDirection: 1 };
      }
    }
    ...
    function writeEventTableState(state) {
      try {
        window.localStorage.setItem(eventTableStateKey(), JSON.stringify(state));
      } catch {
    ...
    function renderTable(id, rows, columns, options = {}) {
      ...
      const expandedRows = new Set(options.details ? eventTableState.expanded : []);
      let sortColumn = options.details && columns.includes(eventTableState.sortColumn) ? eventTableState.sortColumn : columns[0];
      ...
      function draw() {
        ...
        const body = '<tbody>' + sortedRows.map((row, index) => renderTableRow(row, columns, index, options, expandedRows.has(eventRowKey(row)))).join('') + '</tbody>';
    ...
    function renderTableRow(row, columns, index, options, expanded = false) {
      ...
      return '<tr><td><button class="event-toggle" type="button" aria-expanded="' + String(expanded) + '" data-detail="' + detailId + '" data-event-key="' + html(eventRowKey(row)) + '">' + (expanded ? '-' : '+') + '</button></td>' + cells + '</tr><tr class="event-detail-row" data-detail-row="' + detailId + '"' + (expanded ? '' : ' hidden') + '><td colspan="' + (columns.length + 1) + '"><div class="event-detail">' + detail + '</div></td></tr>';
    }
